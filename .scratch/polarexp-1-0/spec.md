...
PolarExp already covers fast navigation, Grid interaction, filename search, file operations,
...
The user wants PolarExp to replace Nautilus on Linux while keeping its Vim-first advantage. The
...
Build PolarExp 1.0 as a local Linux file manager for Wayland and X11. Keep the standard desktop
...
Every mutation reports what succeeded and what failed. PolarExp must not silently overwrite an
entry, discard metadata, claim an unsafe Undo, or leave a partial destination looking complete.

## User Stories

1. As a Linux user, I want PolarExp to work on Wayland and X11, so that my file manager does not depend on one compositor.
2. As a desktop user, I want familiar mouse and keyboard controls, so that I can use PolarExp without learning Vim first.
...
6. As a Nautilus user, I want to paste files copied or cut in Nautilus into PolarExp, so that switching file managers does not break my clipboard.
7. As a Dolphin user, I want to paste files copied or cut in Dolphin into PolarExp, so that KDE clipboard conventions remain compatible.
8. As a PolarExp user, I want Nautilus and Dolphin to understand PolarExp Copy and Cut, so that the clipboard works in both directions.
9. As a user, I want a malformed or ambiguous external clipboard to fall back to Copy, so that PolarExp never guesses that it may delete a source.
10. As a user, I want PolarExp to reject unsupported remote clipboard URIs, so that a local file operation does not misinterpret a network resource.
...
14. As a user whose clipboard ownership changes, I want pending Cut entries restored, so that PolarExp does not keep hiding valid files.
15. As a user pasting a Cut into its source folder, I want a no-op, so that PolarExp does not create a duplicate by mistake.
...
30. As a user entering a multi-key command, I want the bottom bar to show the pending sequence, so that I know what PolarExp expects next.
...
41. As a user who made a mistake, I want persistent Undo for Rename, Move, Trash, New Folder, and Copy, so that recovery survives restarting PolarExp.
42. As a user who undoes a Copy, I want PolarExp to remove the result only when it is unchanged, so that later edits are protected.
43. As a user who undoes New Folder, I want PolarExp to remove it only when it is empty, so that later content is protected.
...
54. As a user whose current folder disappears, I want PolarExp to navigate to the nearest existing ancestor and explain why, so that I do not remain in a broken location.
...
60. As a user who cancels Copy, I want PolarExp to remove only its private incomplete results, so that pre-existing files remain untouched.
...
90. As a user who reduces motion, I want animations and high-frequency animation subscriptions disabled, so that PolarExp respects the desktop preference.
91. As a command-line user, I want `polarexp` to accept a path or local file URI, so that other applications can open a location directly.
92. As a user reopening PolarExp, I want window geometry, the last folder, views, sorting, Favorites, and Undo history restored, so that the application resumes useful state.
...
94. As a Linux desktop user, I want a desktop entry, icons, AppStream metadata, Flatpak, and a regular binary archive, so that PolarExp installs and launches normally.
95. As a user running `!`, I want the command to run directly with my permissions, so that PolarExp keeps its current power-user command workflow.
...
- Keep PolarExp on Rust and Iced for 1.0. Do not add an Iced accessibility fork.
