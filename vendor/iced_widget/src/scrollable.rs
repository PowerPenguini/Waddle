    ...
    fn new(
        ...
        let y_scrollbar = if let Some(vertical) = show_scrollbar_y {
            ...
            let scroller = if ratio >= 1.0 {
                ...
                let scroller_offset = (translation.y
                    / (content_bounds.height - bounds.height))
                    .clamp(0.0, 1.0)
        ...
        let x_scrollbar = if let Some(horizontal) = show_scrollbar_x {
            ...
            let scroller = if ratio >= 1.0 {
                ...
                let scroller_offset = (translation.x
...
mod waddle_scrollbar_tests {
    ...
    fn minimum_thumb_stays_inside_track_and_dragging_preserves_position() {
        ...
        for anchor in [Anchor::Start, Anchor::End] {
            let direction = Direction::Both {
                vertical: Scrollbar::new().anchor(anchor),
                horizontal: Scrollbar::new().anchor(anchor),
            };
            for percentage in [0.0, 0.5, 1.0] {
                ...
                let scrollbars =
                    Scrollbars::new(&state, direction, bounds, content);
                let vertical = scrollbars.y.unwrap();
                let horizontal = scrollbars.x.unwrap();
                let y_thumb = vertical.scroller.unwrap().bounds;
                let x_thumb = horizontal.scroller.unwrap().bounds;
                assert_eq!(y_thumb.height, 32.0);
                assert_eq!(x_thumb.width, 32.0);
                assert!(y_thumb.y >= vertical.bounds.y);
                assert!(
                    y_thumb.y + y_thumb.height
                        <= vertical.bounds.y + vertical.bounds.height + 0.001
                );
                assert!(x_thumb.x >= horizontal.bounds.x);
                assert!(
                    x_thumb.x + x_thumb.width
                        <= horizontal.bounds.x
                            + horizontal.bounds.width
                            + 0.001
                );
                let y = vertical.scroll_percentage_y(
                    0.3,
                    Point::new(
                        y_thumb.center_x(),
                        y_thumb.y + y_thumb.height * 0.3,
                    ),
                );
                let x = horizontal.scroll_percentage_x(
                    0.3,
                    Point::new(
                        x_thumb.x + x_thumb.width * 0.3,
                        x_thumb.center_y(),
                    ),
                );
                assert!(
                    (y - percentage).abs() < 0.001,
                    "vertical drag jumped: {y}"
                );
                assert!(
                    (x - percentage).abs() < 0.001,
                    "horizontal drag jumped: {x}"
                );
            }
        }
    }
    ...
    #[test]
    fn tiny_tracks_keep_the_thumb_in_bounds_and_dragging_finite() {
        for size in [0.0, 1.0, 20.0, 32.0] {
            let bounds = Rectangle {
                x: 0.0,
                y: 0.0,
                width: 100.0,
            ...
            let vertical = scrollbars.y.unwrap();
            let thumb = vertical.scroller.unwrap().bounds;
            assert_eq!(thumb.height, size);
            assert!(
                vertical
                    .scroll_percentage_y(0.5, thumb.center())
                    .is_finite()
            );
        }
    }
